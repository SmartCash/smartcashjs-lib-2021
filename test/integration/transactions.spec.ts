...
describe('smartcashjs-lib (transactions with psbt)', () => {
    ...
    it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2MS(2 of 4)) (multisig) input', async () => {
        ...
        assert.strictEqual(psbt.validateSignaturesOfInput(0), true);
        assert.strictEqual(psbt.validateSignaturesOfInput(0, multisig.keys[0].publicKey), true);
        assert.throws(() => {
    ...
    it(
        ...
        async () => {
            ...
            assert.strictEqual(psbt.validateSignaturesOfInput(0), true);
            assert.strictEqual(psbt.validateSignaturesOfInput(0, p2sh.keys[3].publicKey), true);
            assert.throws(() => {
    ...
    it('can create (and broadcast via 3PBP) a Transaction, w/ a ' + 'P2SH(P2MS(2 of 2)) input with nonWitnessUtxo', async () => {
        const myKey = bitcoin.ECPair.makeRandom({ network: regtest });
        const myKeys = [myKey, bitcoin.ECPair.fromPrivateKey(myKey.privateKey!, { network: regtest })];
