...
export class TransactionBuilder {
    ...
    private __overMaximumFees(bytes: number): boolean {
        ...
        const outgoing = this.__TX.outs.reduce((a, x) => a + (x as Output).value, 0);
...
function trySign({ input, ourPubKey, keyPair, signatureHash, hashType, useLowR }: SigningData): void {
