...
export class Psbt {
    ...
    constructor(opts: PsbtOptsOptional = {}, readonly data: PsbtBase = new PsbtBase(new PsbtTransaction())) {
        // set defaults
    ...
    finalizeInput(inputIndex: number, finalScriptsFunc: FinalScriptsFunc = getFinalScripts): this {
    ...
    signAllInputsHDAsync(hdKeyPair: HDSigner | HDSignerAsync, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): Promise<void> {
        return new Promise(
            (resolve, reject): any => {
                if (!hdKeyPair || !hdKeyPair.publicKey || !hdKeyPair.fingerprint) {
    ...
    signAllInputs(keyPair: Signer, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): this {
    ...
    signAllInputsAsync(keyPair: Signer | SignerAsync, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): Promise<void> {
        return new Promise(
            (resolve, reject): any => {
    ...
    signInput(inputIndex: number, keyPair: Signer, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): this {
        if (!keyPair || !keyPair.publicKey) throw new Error('Need Signer to sign input');
...
function hasSigs(neededSigs: number, partialSig?: any[], pubkeys?: Buffer[]): boolean {
    ...
    if (pubkeys) {
        sigs = pubkeys
            .map((pkey) => {
                const pubkey = ecPairFromPublicKey(pkey, { compressed: true }).publicKey;
